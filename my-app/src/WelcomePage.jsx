...
function WelcomePage(props) {
  ...
  async function handleDrop(column) {
    ...
    // Check for win
    if (checkForWin(newBoard, row, column)) {
      setWin(true);
      // Display winning message or handle the win scenario
    } else {
    ...
    // Handle a draw scenario (if the board is full)
    if (checkForDraw(newBoard)) {
      // Handle the draw scenario
    }
  }

  function checkForWin(board, row, column) {
        ...
        for (let row = 0; row <= rows - 4; row++) {
          ...
          for (let k = row + 1; k < row + 4; k++) {
            if (board[getIndex(k, column)] === board[start]) {
              ...
              break; // Break out of the loop if a sequence is interrupted
            }
          }
        }
      }
